...
export class SecretRotationManager {
  ...
  static async checkRotationNeeded(): Promise<Secret[]> {
    ...
    try {
      ...
      console.error('[SecretRotation] Error checking rotation:', error);
      return [];
    }
  }
  ...
  static async rotateSecret(secretName: string): Promise<boolean> {
    try {
      console.log(`[SecretRotation] Starting rotation for: ${secretName}`);
      ...
      console.log(`[SecretRotation] Successfully rotated: ${secretName}`);
      return true;
    } catch (error) {
      console.error(`[SecretRotation] Failed to rotate ${secretName}:`, error);
      return false;
    }
  }
  ...
  private static async triggerRedeploy(secretName: string): Promise<void> {
    console.log(`[SecretRotation] Triggering redeploy for: ${secretName}`);
  ...
  private static async recordRotation(secretName: string): Promise<void> {
    ...
    console.log(`[SecretRotation] Recorded rotation for: ${secretName} at ${new Date().toISOString()}`);
  }
  ...
  static async dailyRotationCheck(): Promise<void> {
    console.log('[SecretRotation] Running daily rotation check...');
    const secretsToRotate = await this.checkRotationNeeded();

    if (secretsToRotate.length > 0) {
      console.log(`[SecretRotation] Found ${secretsToRotate.length} secrets needing rotation`);
      
      for (const secret of secretsToRotate) {
        await this.rotateSecret(secret.name);
      }
    } else {
      console.log('[SecretRotation] No secrets need rotation');
    }
  }
}
